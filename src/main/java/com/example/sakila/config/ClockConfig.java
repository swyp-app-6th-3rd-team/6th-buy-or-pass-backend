 ...
 *
 * <p><b>초 단위로 끊는 이유</b> — Sakila 의 시간 컬럼은 {@code datetime(0)}, 즉 초 단위다.
 * {@code LocalDateTime.now()} 는 나노초를 갖기 때문에 저장하면 값이 잘리고,
 * <b>저장 직후 메모리의 객체와 DB 에서 다시 읽은 값이 달라진다.</b>
 *
 * <p>이 불일치는 조용히 데이터를 잃게 만든다. 실제로 keyset 스크롤에서 발견했다 —
 * 커서를 나노초 값으로 만들면 {@code rentalDate > 09:00:00.123456789} 조건이
